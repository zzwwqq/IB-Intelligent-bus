...
import com.bus.view.SchedulingDialog;
...
public class IcClientController implements ActionListener, TabClickListener {
	...
	@Override
	public void onTabClick(ModuleEnum tabEnum) {
		// 重置查询条件
	 ...
	 *            按钮
	 */
	private void clickOperate(BtnEnum btnEnum) {
		JTable table = null;
	...
	private void doAdd(DefaultTableModel tableModel, JTable table) {
		if (table.getCellEditor() == null) {
			...
			case LINE:
				...
				break;
			case SCHEDULING:
				emptyRow = new String[] { "", "请选择", "请选择", "1", "", "请选择", "请选择", "请选择" };
				break;
			default:
				break;
			}
	 ...
	 * @return
	 */
	private void doSave(DefaultTableModel tableModel, JTable table) {
		// 如果表格中有显示在编辑的，则停止编辑
		if (table.getCellEditor() != null) {
	...
	private void doSearch() {
		...
		case STATION:
			...
			for(Station station : stationResult) {
				station.setStationName(station.getStationName().substring(0,station.getStationName().indexOf("[")));
			}
		...
		case SCHEDULING:
			...
			// 查询排班请求
			SchedulingService schedulingService = ServiceFactory.getSchedulingService();
			List<Scheduling> schedulingResult=schedulingService.find(schedulingCondition);
			LineService listLineService = ServiceFactory.getLineService();
			List<Line> listLineResult = listLineService.find(new Line());
			BusService listBusService = ServiceFactory.getBusService();
			List<Bus> listBusResult = listBusService.find(new Bus());
			UserService listUserService = ServiceFactory.getUserService();
			List<User> listUserResult = listUserService.find(new User());
			
			Map<String, Vector<String>> stationLineMap = new HashMap<>();
			Map<String,String> lineMap = new HashMap<>();
			for(Line l : listLineResult) {
				lineMap.put(l.getLineName(), l.getLineCode());
				
				String allStations = l.getAllStation();
				Vector<String> vec = new Vector<>();
				String[] stations = allStations.split(",");
				for (String station : stations) {
					vec.add(station);
				}
				stationLineMap.put(l.getLineName(), vec);
				
			}
			
            Map<String,String> busMap = new HashMap<>();
            for(Bus b : listBusResult) {
            	busMap.put(b.getBusLicense(), b.getBusCode());
            }
			
            Map<String,String> userMap = new HashMap<>();
            for(User u : listUserResult) {
            	userMap.put(u.getLoginName(), u.getCode());
            }
            
			ListSchedulingAndMap map =new ListSchedulingAndMap();
			map.setSchedulingList(schedulingResult);
			map.setLineMap(lineMap);
			map.setBusMap(busMap);
			map.setUserMap(userMap);
			map.setStationLineMap(stationLineMap);
			List<ListSchedulingAndMap> list = new ArrayList<>();
			list.add(map);
			mainView.changeTable(list, currentModule);
			break;
		default:
			break;
		}
	...
	private void doLineStationSearch() {
		// 获取查询条件
	...
	private void schedulingConfirm() {
		String writeSchedulingCode = schedulingDialog.getCode();
		boolean writeResult = NFCUtil.write(writeSchedulingCode);
		if (writeResult) {
			schedulingDialog.dispose();
			JOptionPane.showMessageDialog(loginView, "写卡成功！", Constants.PROMPT, JOptionPane.PLAIN_MESSAGE);
		} else {
			JOptionPane.showMessageDialog(loginView, "写卡失败，请检查读卡器是否连接，卡片识别是否正常！", Constants.PROMPT,
					JOptionPane.ERROR_MESSAGE);
		}
	}

	private void userChangePassConfirm() {
		// 修改密码
		...
		String userCode = mainView.getUserName();
		...
		// 请求修改密码
		UserService userService = ServiceFactory.getUserService();
		userService.changePass(userCode, MD5Util.md5Encode(newPassword));
	...
	/*
	 * public void onMessageReceive(IoSession session, MinaMessage minaMessage) {
	 * LOGGER.info("登录应答返回消息: {}", minaMessage);
	 * 
	 * String messageCode = minaMessage.getMessageCode();
	 * 
	 * // 查询成功 if (Constants.MS2000001.equals(messageCode)) {
	 * mainView.changeTable((List) minaMessage.getData(), currentModule); return; }
	 * // 获取添加-返回成功 if (Constants.MS1000001.equals(messageCode)) {
	 * mainView.changeTable((List) minaMessage.getData(), currentModule);
	 * JOptionPane.showMessageDialog(mainView, minaMessage.getMessageStr(),
	 * Constants.PROMPT, JOptionPane.PLAIN_MESSAGE); return; } // 添加失败 if
	 * (Constants.MS1000002.equals(messageCode)) {
	 * JOptionPane.showMessageDialog(mainView, minaMessage.getMessageStr(),
	 * Constants.PROMPT, JOptionPane.ERROR_MESSAGE); return; } // dialog中筛选按钮查询成功 if
	 * (Constants.MS3000002.equals(messageCode)) {
	 * mainView.getLineStationsDialog().setStationList((List<Station>)
	 * minaMessage.getData()); return; } // 修改密码成功 if
	 * (Constants.MS0010401.equals(messageCode)) { IoSession ioSession =
	 * MinaClient.getIoSession(); ioSession.setAttribute(Constants.KEY_PASSWORD,
	 * MD5Util.md5Encode(changePassDialog.getNewPassword()));
	 * JOptionPane.showMessageDialog(mainView, minaMessage.getMessageStr(),
	 * Constants.PROMPT, JOptionPane.PLAIN_MESSAGE); changePassDialog.dispose();
	 * return; } // 修改密码失败 if (Constants.MS0010402.equals(messageCode)) {
	 * JOptionPane.showMessageDialog(mainView, minaMessage.getMessageStr(),
	 * Constants.PROMPT, JOptionPane.ERROR_MESSAGE); return; }
	 * 
	 * // 读卡时查询成功 if (Constants.MS0070201.equals(messageCode)) { Scheduling
	 * scheduling = minaMessage.getData(); if (scheduling != null) {
	 * schedulingDialog = new SchedulingDialog(mainView, true, this, scheduling,
	 * false); schedulingDialog.setVisible(true); } else {
	 * schedulingDialog.dispose(); JOptionPane.showMessageDialog(mainView,
	 * minaMessage.getMessageStr(), Constants.PROMPT, JOptionPane.ERROR_MESSAGE); }
	 * return; } }
	 */
