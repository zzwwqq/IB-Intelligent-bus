			...
			if (TableModelEvent.UPDATE == e.getType()) {
				...
				case SCHEDULING:
					if (table.getValueAt(row, 0) != null && String.valueOf(table.getValueAt(row, 0)).length() > 0) {
						...
						scheduling.setCode(String.valueOf(table.getValueAt(row, 0)));
						String lineCode = String.valueOf(table.getValueAt(row, 8));
//						lineCode = lineCode.substring(lineCode.indexOf("[")+1,lineCode.indexOf("]"));
						scheduling.setLineCode(lineCode);
						String busCode = String.valueOf(table.getValueAt(row, 2));
//						busCode = busCode.substring(busCode.indexOf("[")+1,busCode.indexOf("]"));
						scheduling.setBusCode(busCode);
						scheduling.setTcNumber(String.valueOf(table.getValueAt(row, 3)));
						scheduling.setTcTime(String.valueOf(table.getValueAt(row, 4)));
						String driverCode = String.valueOf(table.getValueAt(row, 9));
//						driverCode = driverCode.substring(driverCode.indexOf("[")+1,driverCode.indexOf("]"));
						scheduling.setUserCode(driverCode);
						String beginStationCode =String.valueOf(table.getValueAt(row, 6));
						beginStationCode = beginStationCode.substring(beginStationCode.indexOf("[")+1,beginStationCode.indexOf("]"));
						scheduling.setStartStation(beginStationCode);
						String endStationCode = String.valueOf(table.getValueAt(row, 7));
						endStationCode = endStationCode.substring(endStationCode.indexOf("[")+1,endStationCode.indexOf("]"));
						scheduling.setEndStation(endStationCode);
						
						boolean isSet = false;
						for (int i = 0; i < updateList.size(); i++) {
							Scheduling updateScheduling = (Scheduling) updateList.get(i);
							if (updateScheduling.getCode().equals(schedulingCode)) {
								updateList.set(i, scheduling);
								isSet = true;
								break;
							}
						}
						if (!isSet) {
							updateList.add(scheduling);
						}
					}
					break;
				default:
