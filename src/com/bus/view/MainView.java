...
public class MainView extends JFrame {
	...
	public void initialize() {
		...
		List<ComboBoxDto> datas = new ArrayList<>();
		datas.add(new ComboBoxDto("2", "全部"));
		datas.add(new ComboBoxDto("1", "启用"));
		datas.add(new ComboBoxDto("0", "禁用"));
	
		conditionUserStatus = new SearchConditionPanel(TextEnum.LABLE_USERSTATUS.getName(), datas);
		...
		conditionBusStatus = new SearchConditionPanel(TextEnum.LABEL_BUSSTATUS.getName(), datas);
		
		//线路查询条件
		conditionLineName = new SearchConditionPanel(TextEnum.LABEL_LINENAME.getName());
		conditionLineStatus = new SearchConditionPanel(TextEnum.LABEL_LINESTATUS.getName(), datas);
	 ...
	 */
	public void createSchedulingDataTab() {
		centerPanel.removeAll();
		// 设置tab标题位置
		currentTabbedPane = new JTabbedPane(JTabbedPane.TOP);
		// 设置tab布局
	 ...
	 */
	public void createUserManagerDataTab() {
		centerPanel.removeAll();
		// 设置tab标题位置
		currentTabbedPane = new JTabbedPane(JTabbedPane.TOP);
		// 设置tab布局
	 ...
	 */
	public void changeTable(List<?> data, ModuleEnum moduleEnum) {
	 ...
	 * @return 含查询条件的实体
	 */
	public Object getSearchCondition(ModuleEnum moduleEnum){
		switch (moduleEnum) {
		case USER:
			User user = new User();
